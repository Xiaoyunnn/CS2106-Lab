...
void *mymalloc(size_t size) {
    ...
    // insert_node(&_memlist, node, ASCENDING);
    // if ((node->next != NULL) 
    //     && (node->next->pdata->status == 0) 
    //     && (node->next->pdata->len > _memlist->max)) {
    //     _memlist->index = node->next->key;
    //     _memlist->max = node->next->pdata->len;
    // }
...
void myfree(void *ptr) {
    ...
    if (node != NULL) {
        ...
        if (node->pdata->len > _memlist->max) {
            _memlist->index = node->key;
            _memlist->max = node->pdata->len;
        }
